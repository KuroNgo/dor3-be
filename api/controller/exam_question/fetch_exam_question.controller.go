...
func (e *ExamQuestionsController) FetchManyExamQuestions(ctx *gin.Context) {
	...
	exam, err := e.ExamQuestionUseCase.FetchManyInAdmin(ctx, page)
	if err != nil {
...
func (e *ExamQuestionsController) FetchOneExamQuestionsByID(ctx *gin.Context) {
	...
	question, err := e.ExamQuestionUseCase.FetchQuestionByIDInAdmin(ctx, questionID)
	if err != nil {
		ctx.JSON(http.StatusBadRequest, gin.H{
			"status":  "error",
			"message": err.Error(),
...
func (e *ExamQuestionsController) FetchOneExamQuestionsByExamID(ctx *gin.Context) {
	...
	exam, err := e.ExamQuestionUseCase.FetchOneByExamIDInAdmin(ctx, examID)
	if err != nil {
		ctx.JSON(http.StatusBadRequest, gin.H{
			"status":  "error",
			"message": err.Error(),
...
func (e *ExamQuestionsController) FetchManyExamQuestionsByExamID(ctx *gin.Context) {
	...
	examID := ctx.Query("exam_id")
	page := ctx.DefaultQuery("page", "1")
	exam, err := e.ExamQuestionUseCase.FetchManyByExamIDInAdmin(ctx, examID, page)
	if err != nil {
		ctx.JSON(http.StatusBadRequest, gin.H{
			"status":  "error",
			"message": err.Error(),
