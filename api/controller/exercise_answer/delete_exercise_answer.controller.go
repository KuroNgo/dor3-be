...
func (e *ExerciseAnswerController) DeleteOneAnswer(ctx *gin.Context) {
...
func (e *ExerciseAnswerController) DeleteAllAnswerInExerciseID(ctx *gin.Context) {
	currentUser, exists := ctx.Get("currentUser")
	if !exists {
		ctx.JSON(http.StatusUnauthorized, gin.H{
			"status":  "fail",
	...
	err = e.ExerciseAnswerUseCase.DeleteOne(ctx, examID)
	if err != nil {
		ctx.JSON(http.StatusInternalServerError, gin.H{
			"status":  "error",
			"message": err.Error(),
